...
- 完成后更新 `handoff.md`
- 完成后停止，不要继续下一步
- 如果当前步骤是最后一个未完成步骤，完成后必须明确告知用户：请确认当前执行结果；确认后本任务无后续规划步骤，不需要再生成下一步 Prompt
...
如果没有下一个未完成步骤，必须进入任务收尾分支：

- 不要生成新的下一步 Prompt
- 更新 `handoff.md`，将当前状态记录为“等待人工确认”或“已完成”；如果需要最终确认，在备注中说明这是任务收尾确认
- 更新 `project-progress-ledger.md` 中该任务状态和最近完成事项；最终确认前总账状态保持“进行中”并备注“等待收尾确认”，用户确认后再改为“已完成”
- 明确通知用户先确认当前执行结果
- 明确告知用户：当前任务的规划步骤已全部执行完，确认后无其他后续步骤
...
5. 输出验证结果
6. 如果当前步骤是最后一个未完成步骤，告知用户确认当前执行结果，并明确确认后无其他后续步骤
7. 停止，不继续下一步
...
如果没有下一个未完成步骤，必须停止生成 Prompt，并输出任务收尾提示：

- 请用户确认当前执行结果
- 告知用户当前任务所有规划步骤已执行完
- 告知用户确认后无其他后续步骤
- 必要时将 `handoff.md` 更新为“等待人工确认”或“已完成”；最终确认前 `project-progress-ledger.md` 状态保持“进行中”并备注“等待收尾确认”，用户确认后再改为“已完成”
